...
export async function generateShareLink(orgId: number, datasetId: number, createdBy: number) {
  const summary = await aiSummariesQueries.getCachedSummary(orgId, datasetId);
  if (!summary) {
  ...
  const share = await sharesQueries.createShare(orgId, datasetId, tokenHash, snapshot, createdBy, expiresAt);
...
export async function getSharedInsight(rawToken: string) {
  ...
  const share = await sharesQueries.findByTokenHash(tokenHash);
  ...
  await sharesQueries.incrementViewCount(share.id);
