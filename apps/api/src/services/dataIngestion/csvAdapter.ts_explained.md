...
### Decision 2: Sample-based row validation (100 rows max)
...
### Decision 3: Synchronous parsing with csv-parse/sync

**What's happening:** The entire CSV is parsed in one blocking call. No streaming, no chunks, no callbacks.

**Why this matters:** The file is already fully in memory as a Buffer from multer's `memoryStorage()`. There's no I/O to make async. Sync parsing is simpler — no callback chains, no backpressure management. The 50,000 row limit bounds worst-case to roughly 200ms of blocking, which is acceptable for a dashboard that isn't handling thousands of concurrent uploads.

**How to say it in an interview:** "Sync parsing is justified here because the file is already buffered in memory and the row limit bounds CPU time. If we needed to support larger files or higher concurrency, I'd switch to streaming with csv-parse's async API and move the work to a worker thread."

**Over alternative:** Streaming would prevent event loop blocking but adds complexity for a bounded problem.

### Decision 4: User-facing error messages, not developer codes

**What's happening:** Error messages say things like "We expected a column named 'date'. Your file has columns: Date, Amount." Not "MISSING_REQUIRED_HEADER: date" or "ValidationError: column not found."

**Why this matters:** The target users are small business owners uploading CSVs from QuickBooks, not developers reading stack traces. Each error tells the user what was expected, what was found, and where the problem is. Row numbers use `i + 2` to match the user's spreadsheet (header is row 1, data starts row 2).

**How to say it in an interview:** "Error messages are written for the end user, not the developer. Each one includes what we expected, what we found, and the exact row number matching their spreadsheet view. That's a product decision baked into the code."
...
**Why this matters:** Rejecting an entire 10,000-row file because 5 rows have typos is a terrible UX. The adapter lets the good data through and clearly communicates what was dropped. The user can fix those rows and re-upload, or accept the partial result.

**How to say it in an interview:** "The adapter recovers gracefully from partial failures. Bad rows are filtered out using a Set for O(1) membership checks, the valid subset is returned, and warnings describe exactly which rows were skipped. It's the difference between 'upload failed' and 'upload succeeded with 5 rows skipped.'"
...
Five configuration values govern the adapter's behavior. `REQUIRED_COLUMNS` and `OPTIONAL_COLUMNS` define the schema contract — what a CSV must have and what it may have. `ALL_KNOWN_COLUMNS` combines them for the header map builder. `MAX_ROWS` caps file size at 50,000 to prevent memory exhaustion. `VALIDATION_SAMPLE_SIZE` limits row-level validation to 100 rows.
...
- **`stripBom`** — Checks if the first character is the Unicode BOM (`U+FEFF`) and slices it off. Excel on Windows prepends this to UTF-8 files. Without stripping, the first header becomes `\uFEFFdate` instead of `date` and fails the column check.
...
### Block 6: csvAdapter.parse (lines 124-196)

The main pipeline. Five guard clauses handle edge cases in sequence:
1. Empty file → warning
2. No data rows (headers only) → warning
3. Too many rows (>50K) → warning
4. Missing required columns → silent rejection (errors come from `validate`)
5. >50% row failure rate → silent rejection

If all guards pass, bad rows are filtered out via a `Set` for O(1) lookup, warnings are built for skipped rows, and the valid subset is returned.

### Block 7: csvAdapter.validate (lines 198-200)

Thin wrapper around `validateHeaders`. Exists because the `DataSourceAdapter` interface requires it as a separate method — the route handler calls it independently to check headers before committing to the full parse.

### Block 8: Test exports (line 204)

Utility functions exported for direct unit testing. This is convention, not interface — production code imports `csvAdapter`, test code can also import individual helpers.

---

## 4. Complexity and Trade-offs

**Sample validation is a bet.** Checking 100 rows assumes data quality problems are consistent. If a CSV has problems only in rows 40,000-50,000 (pasted from a different system), the sample won't catch it. For SMB-scale data from single exports, this is acceptable. Full-scan validation would catch everything but adds latency proportional to file size.
...
**Where it appears:** Lines 128, 143, 152, 165, 173 — five early returns before the happy path.
...
"Be conservative in what you send, be liberal in what you accept." BOM markers? Strip them. Uppercase headers? Normalize them. Commas in numbers? Remove them. Extra columns? Ignore them. The output is strict and consistent; the input is as permissive as possible.

**Where it appears:** `stripBom`, `normalizeHeader`, `isValidAmount` (comma stripping), `relax_column_count` option.

**Interview-ready line:** "The adapter follows Postel's Law — liberal in what it accepts, conservative in what it produces. It normalizes BOM markers, header casing, number formatting, and inconsistent column counts so the downstream pipeline gets clean data regardless of input quality."
...
### Q2: "How would you handle a 500,000-row file?"

**Strong answer:** "Three changes. First, streaming CSV parsing instead of sync. Second, move the work to a background job via BullMQ so the endpoint returns immediately with a job ID. Third, batch database inserts in chunks of 1,000-5,000 rows. The validation sampling still works — you'd check the first 100 rows regardless of total count."

**Red flag answer:** "Just remove the MAX_ROWS limit." — Ignores memory, CPU, and database implications.

### Q3: "What's the BOM marker and why strip it?"

**Strong answer:** "The Byte Order Mark is `U+FEFF`, a zero-width character that Excel on Windows prepends to UTF-8 files. It was originally for UTF-16 byte order signaling but is meaningless in UTF-8. Without stripping, the first header becomes `\uFEFFdate` and fails the column check. One character check, zero cost, prevents a baffling error for users who don't know what encoding is."
...
**Strong answer:** "Check 100 rows, reject if >50 fail. The idea is high failure rate signals a structural problem — wrong format, wrong file. Whether 50% is right is empirical. I'd log failure rates for every upload and whether users re-upload after rejection. If users get rejected at 55% but the data was mostly usable, the threshold is too strict."
...
### Q6: "Why export utility functions separately?"

**Strong answer:** "For unit testing. Testing `isValidDate` directly is faster and more targeted than routing through the full parse pipeline. The adapter object is the production API. The utility exports are a testing convenience — internal helpers that aren't part of the interface."

**Red flag answer:** "Everything should be exported." — Conflates internal helpers with public API surface.
...
**Complexity:** O(k) to build where k is header count (3-5). O(1) per lookup.
...
**Where it appears:** Line 187 — `const skippedSet = new Set(skippedRows)` used in `records.filter()`.

**Why this one:** The filter runs over every row (up to 50,000). With an Array, each `includes()` is O(s). With a Set, each `.has()` is O(1). For 50,000 rows with 50 skipped, that's 2.5M comparisons vs. 50,000 hash lookups.
...
`const rowNum = i + 2` accounts for 0-indexed arrays and the header row. If error messages said "Row 0" or "Row 1," users would look at the wrong row in Excel. Getting this right is a two-minute fix that prevents a support ticket every time someone debugs a validation error.

**How to bring it up:** "Row numbers use `i + 2` to match spreadsheet numbering. The user thinks in Excel rows; the code thinks in array indices. The +2 translates between those two worlds."
