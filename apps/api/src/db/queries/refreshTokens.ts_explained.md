...
**`findByHash` is O(log n)** via the unique index on `tokenHash`. The additional conditions (`revokedAt IS NULL`, `expiresAt > NOW()`) are checked after the index lookup, so they don't add significant cost.
...
**Interview-ready line:** "Soft delete via `revokedAt` preserves the audit trail while making revoked tokens invisible to validity queries. The timestamp captures when revocation happened, which is critical for security incident timelines."
...
### The Audit Trail Serves Security Forensics
