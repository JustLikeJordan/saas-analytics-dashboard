...
**Lines 39-48: SSE passthrough.** The key line: `new Response(upstream.body, { status: 200, headers: {...} })`. `upstream.body` is a `ReadableStream<Uint8Array>` — we hand it directly to the browser. The headers explicitly set SSE content type, no caching, and keep-alive. The browser's `fetch` in `useAiStream` reads this stream chunk by chunk.
...
- SSE: `Content-Type: text/event-stream` with a `ReadableStream` body
